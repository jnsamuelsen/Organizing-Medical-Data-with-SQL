...
CREATE TABLE PatientInfo (ID INT PRIMARY KEY, Age INT, Sex CHAR(1), Status TEXT, N_Days INT, Stage INT, Drug VARCHAR);
...
COPY patientinfo
FROM '/tmp/cirrhosis_PatientInfo.csv' DELIMITER ',' CSV HEADER;

COPY histology
FROM '/tmp/cirrhosis_Histology.csv' DELIMITER ',' CSV HEADER;

COPY chemicalcomp
FROM '/tmp/cirrhosis_ChemicalComp.csv' DELIMITER ',' CSV HEADER;
...
ADD HistologyID SERIAL;

ALTER TABLE histology
ADD PRIMARY KEY (histologyid);

ALTER TABLE chemicalcomp
ADD ChemicalCompID SERIAL;

ALTER TABLE chemicalcomp
ADD PRIMARY KEY (chemicalcompid);

-- Set up the table relations (FOREIGN KEYS)
ALTER table chemicalcomp
ADD CONSTRAINT id
FOREIGN KEY (id)
REFERENCES patientinfo (id);
